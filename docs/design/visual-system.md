...
状态：外壳视觉基线保留；纠偏后的空状态与配置交互待预览审阅。品牌语义为「清晰、可信、高效，少一些操作、多一些完成」。颜色和尺寸的事实来源是 `src/ui/design-tokens.ts`，组件和可交互预览表达实现；本文只维护原则和取舍。
...
现有 `npm run dev` 预览仍含待移除的合成 ERP 与业务文案，不能作为纠偏后设计的交付。后续产品与预览继续复用外壳组件。M1 首先实现浅色，深色与高对比主题后续按同一语义层扩展。
