...
状态：Linux 本地功能通过，macOS 工件与市场收录继续跟踪。运行环境为已授权的本地隔离账号，Electron 实验使用 `--no-sandbox`；这不构成插件沙箱或兼容性承诺。
...
真实 DeepSeek 查询成功：加载 Skill 后调用插件，从首页进入其他收支、展开类别并读取证据。记录为 3 次模型调用、6 次宿主工具调用、约 5.2 秒；未要求确认，未保存、勾选类别或写入业务数据。可见选项不等于遍历所有分组，不声称完整类别目录。这是小样本功能证据，不是普遍效果或性能结论。
...
macOS CI／用户 Mac 的本轮验证、插件公开下载地址、市场收录及安装状态在实现 PR 和 Issue #12 记录；未完成时不以本地夹具替代。原始输出在忽略的 `.artifacts/`，不提交身份或业务数据。
